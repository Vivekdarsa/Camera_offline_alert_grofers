...
from utils import get_time_str, add_offline_log, add_online_log
...
class OfflineAlert(Thread):
    ...
    def run(self):
        ...
        while True:
            ...
            print('**********', datetime.now())
            table_data = ''
            for cam, cam_data in offline_cam_dict.items():
                if cam_data['mailing_status'] is False:
                    table_data += '<tr><td style="border: 1px solid orange">{0}</td> \
                                ...
                                .format(cam_data['name'], self.get_facility_name_from_sector(cam_data['sector']))
                    cam_data['mailing_status'] = True
            if table_data != '':
                print('sending mail to', self.conf_data['company_name'], datetime.now(), self.conf_data['email_addresses'])
                print(send_mail(self.conf_data, html_str(table_data)))


            # sleep(30)
            sleep(int(self.conf_data['alert_duration']))
